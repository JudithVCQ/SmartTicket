...
jest.mock('../../src/lib/ai', () => ({
  categorizeTicketWithGemini: jest.fn<() => Promise<{ categoria: string; prioridad: string }>>().mockResolvedValue({
    categoria: 'Software',
    prioridad: 'Media',
  }),
}));
...
jest.mock('../../src/lib/db', () => ({
  query: jest.fn<(...args: unknown[]) => Promise<{ rowCount: number; rows: Record<string, unknown>[] }>>()
